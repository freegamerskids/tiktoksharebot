    ...
    try:
        ...
        try:
            if (req.json()["status_code"] == 0):
                ...
                TotalSentShare += 1
                Title(f"Thread :{str(active_count() - 2)} / Sent :{TotalSentShare} / Failed :{TotalFailedReq}")
            else:
                pass
        except:
            TotalFailedReq += 1
...
def ClearURI(link):
    if ("vm.tiktok.com" in itemID or "vt.tiktok.com" in itemID):
        return \
            r.head(itemID, stream=True, verify=False, allow_redirects=True, timeout=5).url.split("/")[5].split("?", 1)[
                0]
    else:
